var protocol = require('../packages/react-native-hypercore-protocol')
...
function replicate (feed, opts) {
  ...
  feed.ready(function (err) {
    Promise.resolve(stream).then((streamResolved) => {
      if (err) return streamResolved.destroy(err)
      if (streamResolved.destroyed) return

      var peer = new Peer(feed, opts)
      peer.feed = feed

      streamResolved.feed(feed.key, {peer: peer}).then((feedStream) => {
        peer.stream = feedStream
      ...
      peer.remoteId = streamResolved.remoteId
      streamResolved.setMaxListeners(0)
      streamResolved.on('handshake', function () {
        peer.remoteId = streamResolved.remoteId
      })
    })
  })
  ...
  stream.on = stream.on || ((ev, handler) => {
    Promise.resolve(stream).then((resolvedStream) => {
      resolvedStream.on(ev, handler)
    })
    return stream
  })

  stream.pipe = stream.pipe || ((dest, pipeOpts) => {
    Promise.all([stream, dest]).then(([resolvedStream, resolvedDest]) => {
      resolvedStream.pipe(resolvedDest)
    })
    return dest
  })
