...
function Protocol (opts) {
  ...
  var streamStub = (async () => {
    // opts.id could a Promise
    this.id = await (opts.id || randomBytes(32))
    this.live = !!opts.live
    this.ack = !!opts.ack
  ...
  // add event like methods that allow
  // async part to be resolved without changing API
  streamStub.on = streamStub.on || ((ev, handler) => {
    Promise.resolve(streamStub).then(() => {
      self.on(ev, handler)
    })
    return streamStub
  })

  streamStub.pipe = streamStub.pipe || ((dest, pipeOpts) => {
    Promise.all([streamStub, dest]).then(([resolvedStreamStub, resolvedDest]) => {
      self.pipe(resolvedDest)
    })
    return dest
  })
...
Protocol.prototype.feed = async function (key, opts) {
  if (this.destroyed) return null
  if (!opts) opts = {}

  var dk = opts.discoveryKey || await discoveryKey(key)
  ...
  if (first) {
    ...
    if (this.encrypted) {
      ...
      this._xor = /* async */ sodium.crypto_stream_xor_instance(this._nonce, this.key)
      if (this._remoteNonce) {
        this._remoteXor = /* async */ sodium.crypto_stream_xor_instance(this._remoteNonce, this.key)
      }
    }
  ...
  // make sure "first" pass has resolved _xor and friends promises
  [this._xor, this._remoteXor, this._nonce, feed.nonce] = await Promise.all([this._xor, this._remoteXor, this._nonce, feed.nonce])

  var box = encodeFeed(feed, ch.id)
  if (!feed.nonce && this.encrypted) {
    this._xor.update(box, box)
  }
  this._keepAlive = 0
  this.push(box)
...
// "private" method that is not used anywhere in this file
// not expected to return anything, but rather emit
Protocol.prototype._onhandshake = function (handshake) {
  if (this.remoteId) return

  Promise.resolve(handshake.id || randomBytes(32)).then((remoteId) => {
...
// async function, but not sync return is expected
Protocol.prototype._onopen = async function (id, data, start, end) {
  ...
  if (!this.remoteDiscoveryKey) {
    ...
    if (this.encrypted && this.key && !this._remoteXor) {
      this._remoteXor = await sodium.crypto_stream_xor_instance(this._remoteNonce, this.key)
    }
  }
...
// async side effect via _onopen
Protocol.prototype._onmessage = async function (data, start, end) {
  ...
  if (type === 0) {
    if (ch) ch._onclose()
    // async side effects
    return await this._onopen(id, data, start, end)
  }

  if (!ch) return this._badFeed()
  if (type === 15) return ch._onextension(data, start, end)
  return await ch._onmessage(type, data, start, end)
}
...
Protocol.prototype._parse = function (data, start, cb) {
  return (async () => {
    ...
    while (start < data.length && !this.destroyed) {
      if (this._missing) start = await this._parseMessage(data, start)
...
Protocol.prototype._parseMessage = async function (data, start) {
  ...
  if (end <= data.length) {
    ...
    // async side effect
    await this._onmessage(data, start, end)
    return ret
  }
...
async function discoveryKey (key) {
  var buf = bufferAlloc(32)
  await sodium.crypto_generichash(buf, bufferFrom('hypercore'), key)
  return buf
}

async function randomBytes (n) {
  var buf = bufferAlloc(n)
  await sodium.randombytes_buf(buf)
  return buf
}
